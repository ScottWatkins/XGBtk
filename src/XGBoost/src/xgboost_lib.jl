...
#----------------------parse cv data section added by sw-----------------------
    ...
    err = -9
    bestcvround = missing
    rsv = 4 * length(metrics)
    
    m = permutedims(Matrix(string.(hcat((split.(results, r"[\t|:\+]"))...))))

    if show_stdv == true                                                     

        cvdat = parse.(Float64, (m[:, collect(3:3:size(m,2)) ] ))
        cvstd = parse.(Float64, (m[:, collect(4:3:size(m,2)) ] ))
        cvdathead = vcat([ [i]  for i in (m[1, collect(2:3:size(m,2))]) ]...)
        cvstdhead = vcat([ [i*"-std"]  for i in (m[1, collect(2:3:size(m,2))]) ]...)

    else                                                                           
        cvdat = parse.(Float64, (m[:, collect(3:2:size(m,2)) ]))                    
        cvdathead = vcat([ [i]  for i in (m[1, collect(2:2:size(m,2))]) ]...)          
    end                                                                                        

    if sum(occursin.(r"^cv-test-[m]?logloss$", cvdathead)) > 0

        tllidx = findall(occursin.(r"^cv-test-[m]?logloss$", cvdathead))
        mintest_round = argmin(cvdat[:, tllidx[1]])
        mintest_val = cvdat[mintest_round, tllidx][1]
        
        trllidx = findall(occursin.(r"^cv-train-[m]?logloss$", cvdathead))
        mintrain_val = cvdat[mintest_round, trllidx][1]

        mintrain_std = mintest_std = missing

        if show_stdv == true
            mintest_std = cvstd[mintest_round, tllidx][1]
            mintrain_std = cvstd[mintest_round, trllidx][1]
        end

        bestcvround = (mintest_round, mintest_val, mintest_std,  mintrain_val, mintrain_std)

        printstyled("Best cv-test-[m]logloss: $mintest_val +/- $mintest_std, round: $mintest_round, train-[m]logloss: $mintrain_val +/- $mintrain_std\n", color=:green)
    ...
    elseif in("binary:logistic", values(Dict(param)))
        ...
        printstyled("Did not find objective in param dictionary.\nError rate not evaluated.\nPlease pass param dictionary with objective to runCVtrain.\n", color=:yellow)
    end
    ...
    return (pred, cvdat, cvdathead, err, bestcvround)        
